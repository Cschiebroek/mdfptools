...
class VacuumSimulator(BaseSimulator):
    @classmethod
    def via_openmm(cls, parmed_obj, file_name, file_path="./", platform="CUDA", num_steps=5000 * 500, write_out_freq=5000, report_production=True,
                constrain_all_bonds=True, **kwargs):
        """
        Runs simulation using OpenMM in vacuum.
        ...
        constrain_what_bond = app.AllBonds if constrain_all_bonds else app.HBonds
        system = pmd.createSystem(nonbondedMethod=app.NoCutoff, constraints=constrain_what_bond)

        # Optionally add thermostat for temperature control
        if hasattr(cls, 'temperature'):
            logger.info("Adding thermostat")
            thermostat = AndersenThermostat(cls.temperature, 1 / unit.picosecond)
            system.addForce(thermostat)
